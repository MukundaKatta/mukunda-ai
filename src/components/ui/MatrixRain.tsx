...
/**
 * Classic 01 matrix rain, tuned thin + palette-synced for this app.
 *
 * Design notes:
 *  - Only 0 and 1 glyphs (binary authenticity)
 *  - Wide column gaps = airy, not a solid wall
 *  - Head glyph in indigo tint (not pure white) so it never competes
 *    with foreground text/portrait
 *  - Trail fade uses #030308 so canvas doesn't stain section bg
 *  - DPR-aware, auto-resizes, respects prefers-reduced-motion
 */
export function MatrixRain({
  ...
  useEffect(() => {
    ...
    // Thin rain — wallpaper, wide gaps, restrained opacity.
    const cfg = {
    ...
    const CHARS = '01'
    const FONT_SIZE = 16
    const COL_GAP = FONT_SIZE * cfg.step
    ...
    const draw = (t: number) => {
      ...
      for (let i = 0; i < columns; i++) {
        ...
        // Head — indigo-tinted, no shadowBlur (would bring it forward)
