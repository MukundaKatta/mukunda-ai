...
const categoryColors: Record<string, { text: string; bg: string; border: string; accent: string }> = {
  ...
  'Backend & APIs': { text: 'text-indigo-600 dark:text-indigo-400', bg: 'bg-indigo-50 dark:bg-indigo-900/20', border: 'border-indigo-200 dark:border-indigo-800/50', accent: 'group-hover:border-indigo-300 dark:group-hover:border-indigo-700' },
  ...
  'Databases': { text: 'text-fuchsia-600 dark:text-fuchsia-400', bg: 'bg-fuchsia-50 dark:bg-fuchsia-900/20', border: 'border-fuchsia-200 dark:border-fuchsia-800/50', accent: 'group-hover:border-fuchsia-300 dark:group-hover:border-fuchsia-700' },
  'Observability & Quality': { text: 'text-violet-600 dark:text-violet-400', bg: 'bg-violet-50 dark:bg-violet-900/20', border: 'border-violet-200 dark:border-violet-800/50', accent: 'group-hover:border-violet-300 dark:group-hover:border-violet-700' },
}
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {skillCategories.map((cat, i) => {
            const colors = categoryColors[cat.title] || { text: 'text-indigo-600 dark:text-indigo-300', bg: 'bg-indigo-50 dark:bg-indigo-500/10', border: 'border-indigo-200 dark:border-indigo-800/50', accent: '' }
            return (
              <ScrollReveal key={cat.title} delay={i * 0.06}>
                ...
                >
                  <div className="flex items-center gap-2 mb-4">
                    <div className={`w-2 h-2 rounded-full`} style={{ backgroundColor: colors.text.includes('blue') ? '#3b82f6' : colors.text.includes('amber') ? '#d97706' : colors.text.includes('cyan') ? '#0891b2' : colors.text.includes('fuchsia') ? '#c026d3' : colors.text.includes('purple') ? '#9333ea' : colors.text.includes('rose') ? '#e11d48' : colors.text.includes('violet') ? '#7c3aed' : colors.text.includes('indigo') ? '#6366f1' : '#6366f1' }} />
