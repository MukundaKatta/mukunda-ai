...
const statusStyles: Record<string, string> = {
  'open-source': 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800/50',
...
export function Projects() {
  return (
    <section id="projects" className="relative py-28 px-6 bg-slate-50/50 dark:bg-slate-900/50 overflow-hidden">
      ...
      <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-emerald-500/[0.03] rounded-full blur-[100px] -translate-y-1/4 translate-x-1/4 pointer-events-none" />
            ...
            <ScrollReveal key={project.name} delay={i * 0.06}>
              ...
              >
                ...
                <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none">
                  <div className="absolute -top-20 -right-20 w-40 h-40 bg-emerald-500/[0.06] rounded-full blur-[40px]" />
                </div>
                ...
                <div className="relative flex items-start justify-between mb-3">
                  <div>
                    ...
                    <p className="text-sm text-emerald-600 dark:text-emerald-400 font-medium">{project.tagline}</p>
                  </div>
                ...
                <div className="relative flex gap-4 pt-3 border-t border-slate-100 dark:border-slate-700/50">
                  {project.github && (
                    <a href={project.github} target="_blank" rel="noopener" className="flex items-center gap-1.5 text-sm text-slate-500 dark:text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors font-medium">
                      <GithubIcon size={16} /> Code
                    </a>
                  )}
                  {project.live && (
                    <a href={project.live} target="_blank" rel="noopener" className="flex items-center gap-1.5 text-sm text-slate-500 dark:text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors font-medium">
